...
function goTo(p){
  ...
  const t={scan:'Scan Barcode',dash:'Dasbor',set:'Pengaturan',users:'Pengguna',master:'Master Produk',tema:'Pilih Tema'};
...
function renderResult({bc,master}){
  ...
  $('#pf').addEventListener('submit',async e=>{
    ...
    const rec={
      ...
      scannedBy:currentUser?currentUser.username:'unknown',
...
async function changeUserRole(username){
  ...
  if(username===currentUser.username){toast('Tidak bisa ubah role sendiri','er');return}
  let target=null;
  try{
    ...
    try{
      ...
      slice.forEach(row=>{
        ...
        batch.set(ref,{bc:String(bc),nm:nm||'',patternCode:pcode||0,returnH:retH||0,
          updatedAt:new Date().toISOString(),updatedBy:currentUser?currentUser.username:'system'});
      });
      await batch.commit();
      done+=slice.length;
